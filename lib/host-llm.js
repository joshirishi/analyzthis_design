...
function submitResponse(runDir, stepId, text) {
  ensureDir(path.join(runDir, 'responses'));
  fs.writeFileSync(responsePath(runDir, stepId), text);
 ...
 */
async function callHostLlm({ personaId, system, user, runContext }) {
  ...
  if (typeof hostResponder === 'function') {
    const text = await hostResponder({ personaId, system, user, stepId, runDir });
    if (text) {
      submitResponse(runDir, stepId, text);
      return text;
    }
  }
