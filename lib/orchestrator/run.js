...
function resolveModel({ manifest, effort, chain, config, defaultProvider, defaultModel, gateName }) {
  ...
  return { provider, model, maxTokens, effort: effKey };
}
...
async function callLlm({ provider, model, system, user, maxTokens }) {
  ...
  // Google Gemini — OpenAI-compatible endpoint. Auth via GEMINI_API_KEY (or GOOGLE_API_KEY).
  if (provider === 'google') {
  ...
  // Z.AI (Zhipu GLM) — OpenAI-compatible endpoint. Auth via ZAI_API_KEY (or ZHIPU_API_KEY).
  if (provider === 'zai') {
  ...
  throw new Error(`Unknown provider "${provider}". Use anthropic, openai, google, or zai.`);
}
...
async function run(opts = {}) {
  ...
  // Detect a delta follow-up: a REVISE verdict already recorded means this run
  // is re-grading only flagged sections, not a fresh full audit.
  ...
  for (const id of experts) {
    ...
    console.log(`\n⏳ Running persona: ${id}...`);
    // Effort-graded model selection (v1.10): classify this sub-task's effort,
    // then resolve provider/model/maxTokens from effort_overrides → effort_matrix
    // → legacy tiers → manifest cap. Persona steps pass gateName=null; gates pin
    // to hard via chain.gate_override.
    const effort = classifyEffort({ routing, digest: state.digest, manifest, scopedMode: null, fullChain: full_chain, isDeltaFollowUp });
    const { provider: callProvider, model: callModel, maxTokens, effort: resolvedEffort } = resolveModel({
      manifest, effort, chain, config, defaultProvider: provider, defaultModel: model, gateName: null,
    });
    ...
    inputTokensEst += inTok;
    outputTokensEst += outTok;
    // Running $ estimate from the pricing config (best-effort; missing pricing → 0).
    const price = config.pricing && config.pricing[callModel];
    if (price) costUsd += (inTok / 1e6) * (price.input_per_m || 0) + (outTok / 1e6) * (price.output_per_m || 0);
    effortLog.push({ persona: id, effort: resolvedEffort, model: callModel, scoped_mode: null });
    console.log(`✅ ${id} complete (${text.length} chars, ${resolvedEffort})`);
  }
...
module.exports = { run, classifyProblem, classifyProblems, resolveExperts, resolveExpertsForRules, loadManifest, classifyEffort, resolveModel };
