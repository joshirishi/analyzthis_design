...
async function run(opts = {}) {
  ...
  // Evolution readiness hint after a successful run.
  try {
    const em = require('../evolution-metrics');
    const ready = em.checkEvolutionReady(result.project_id || session.getProjectId());
    if (ready.ready) {
      console.log('');
      console.log('   [Evolution] Team has enough data to evolve. Run: npx analyzthis_design evolve --extract');
    } else {
      const agg = ready.metrics.aggregate;
      console.log('');
      console.log(`   [Evolution] ${ready.metrics.aggregate.evolution_level} — ${agg.total_lessons} lessons, ${agg.total_outcomes} outcomes. Accept outputs + confirm outcomes to trigger evolution.`);
    }
  } catch (e) { /* evolution-metrics unavailable */ }
