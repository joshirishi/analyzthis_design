...
async function callLlm({ provider, model, system, user, maxTokens, personaId, hostContext }) {
  if (provider === 'host') {
    return callHostLlm({ personaId, system, user, runContext: hostContext });
  }
...
async function run(opts = {}) {
  ...
  async function loadPersonaContext(personaId, { isProduceRound, isObjectionRound }) {
    ...
    const context = (full_chain || useDeepWireframe || isProduceRound)
      ? loadSkill(manifest).slice(0, 12000)
      : loadCard(manifest, personaId);
    const personaUsesLite = useLiteSchema && !useDeepWireframe && isObjectionRound;
  ...
  try {
    delibResult = await deliberation.runDeliberation({
      ...
      callPersona,
      loadPersonaContext,
      buildReferencePack: (id, t) => {
        const pack = buildReferencePack(id, t);
