...
function detectStack(vaultPath) {
  ...
  const techNotes = fs.readdirSync(path.join(vaultPath, 'Tech')).map((f) => f.toLowerCase());
  const stacks = ['nextjs', 'react', 'shadcn', 'vue', 'angular', 'svelte', 'nuxtjs', 'astro'];
  for (const stack of stacks) {
    if (techNotes.some((n) => n.includes(stack))) return stack;
  }
  return null;
}
...
function getReferenceSpec(id, state) {
  ...
  if (!spec) return null;
  if (id === 'priya') {
    const stack = detectStack(state.vault_path);
...
/**
 * Retrieve a ranked, citation-ready reference pack for one persona's active
 * dimension. Uses query expansion to broaden keyword matching, then a cheap
 * LLM ranker to pick the top-5 persona-specific references from reference
 * rows + per-persona knowledge slices. Falls back to static keywords and
 * unranked results if expansion/ranking fails.
 */
async function buildReferencePack(id, task, state) {
  const spec = getReferenceSpec(id, state);
  if (!spec) return null;

  let keywords = extractKeywords(task);

  try {
    const expanded = await queryExpander.expandQuery({
      task, personaId: id,
      callLlmFn: callLlm,
      provider: 'host',
      model: 'devi',
    ...
    // fall back to static keyword extraction
  }
  ...
  try {
    ...
    try {
      const kbNotes = knowledge.getPersonaSliceForPrompt(state, id);
      for (const note of kbNotes.slice(0, 5)) {
        pool.push({
          type: 'knowledge',
          source: note.title,
          row: 0,
      ...
      // knowledge slices unavailable — continue with references only
    }

    try {
      const ranked = await ranker.rankCandidates({
        personaId: id,
        task,
        candidates: pool,
        callLlmFn: callLlm,
      ...
      return {
        ...
        citations: ranked.ranked.slice(0, 5).map((c) =>
          `[${c.source}${c.row ? ', row ' + c.row : ''}: "${c.content}"]`
        ),
      };
    } catch {
      return {
        cacheHit: result.cacheHit,
        citations: result.rows.slice(0, 5).map((row) => retrieve.cite(spec.file, row, spec.column)),
      };
    }
  } catch {
...
async function callLlm({ provider, model, system, user, maxTokens, personaId, hostContext }) {
  ...
  if (provider === 'anthropic') {
    ...
    throw new Error(`Unknown provider "${provider}". Use anthropic, openai, google, zai, or host.`);
  }
...
module.exports = { run, classifyProblem, classifyProblems, resolveExperts, resolveExpertsForRules, loadManifest, classifyEffort, resolveModel, callLlm, loadConfig };
